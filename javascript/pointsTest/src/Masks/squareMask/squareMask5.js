...
class squareMask5 extends MaskBase {

	//------------------------------------------
	setup() {
		super.addLayer();
		this.name = "squareMask5";
		this.rectWidth = 50;
		// vertical
		...
		this.noseGroup = new paper.Group({
			children: [nose],
			transformContent: false,
			pivot: [this.eye.bounds.width/2, this.eye.bounds.height],
		});
	...
	update(data) {
		...
		this.earLGroup.scaling.y = this.angle+0.2;
		
		this.earRGroup.position = earR.position;
		this.earRGroup.scaling.y = this.angle+0.2;
		
		this.noseGroup.position = nose.position.add([0, 0]);
		this.noseGroup.scaling.y = this.angle+0.2;
		this.noseGroup.scaling.x = this.velocity+0.2;
		...
		this.mouthGroup2.scaling.x = 2.15;
